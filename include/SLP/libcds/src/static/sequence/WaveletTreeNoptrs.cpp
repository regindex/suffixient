...
//        occ = new BitSequenceDArray(oc,new_n+1);
	occ = new BitSequenceSDArray(oc,new_n+1);
        delete [] occurrences;
        this->n = new_n;
...
//        occ = new BitSequenceDArray(oc,new_n+1);
	...
	size_t before=0;
        while(level<height) {
            ...
            if(is_set(symbol,level)) {
                ...
                pos=pos-start+before-bitstring[level]->rank1(pos);
                count=pos+1;
                end=end-start-(bitstring[level]->rank1(end)-before);
                end+=start;
                pos+=start;
    ...
    size_t WaveletTreeNoptrs::count(uint symbol) const
    {
        ...
        uint pos = occ->select1(s-1);

        return occ->selectNext1(s)-pos+1;
    }
...
*/
	// DEVUELVE EL PUNTO EN EL QUE SE CORTAN LOS BITMAPS
	int 
	WaveletTreeNoptrs::rangeQuery(uint level, uint *il1, uint *il2, uint left, uint right)
	{
		if (level < this->height)
		{					
			uint l0 = bitstring[level]->rank0(left-1);
			uint r0 = bitstring[level]->rank0(right);
			
			*il1 = bitstring[level]->rank0(left+*il1-2) - l0 + 1;
			*il2 = bitstring[level]->rank0(left+*il2-1) - l0;
	...
	size_t WaveletTreeNoptrs::map(uint symbol)
	{
		return am->map(symbol);
	}
	
	size_t WaveletTreeNoptrs::unmap(uint symbol)
	{
		return am->unmap(symbol);
	}
	...
	Mapper* WaveletTreeNoptrs::getMapper()
	{
		return am;
	}
};
